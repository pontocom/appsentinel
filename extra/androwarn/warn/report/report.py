...
def generate_report(package_name, data, verbosity, report, output) :
    ...
    # CJCS: Changed this to enable the creation of a report with a different name (using the filename, not the package name)
    # output_file = os.path.join(os_getcwd(), package_name + "_%s" % str(int(time.time()))) if not(output) else output
    output_file = os.path.join(os_getcwd(), package_name) if not (output) else output
